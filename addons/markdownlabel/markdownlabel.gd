...
func _convert_markdown(source_text: String = "") -> String:
	...
	for line: String in lines:
		...
		_processed_line = _process_blockquote_syntax(_processed_line)
...
func _process_blockquote_syntax(line: String) -> String:
	var processed_line := line
	if processed_line.strip_edges().begins_with(">"):
		if not _within_blockquote:
			_within_blockquote = true
			return "    [table=2][cell bg=#fff][/cell][cell bg=#fff1 padding=5,0,0,0]" + processed_line.strip_edges().substr(1).strip_edges().lstrip(">")
		else:
			return processed_line.strip_edges().substr(1).strip_edges().lstrip(">")  # Remove the '>' and return the line
	else:
		if _within_blockquote:
			_within_blockquote = false
			return "[/cell][/table]\n" + processed_line
		else:
			return processed_line
